...
export default function GymHubPage() {
  ...
  const filtered = marketGyms.filter(
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-muted">
              Browse and manage training environments
            </p>
          </div>
          {/* Tabs */}
...
function ComplexityBar({ value }: { value: number }) {
  const color =
    value >= 90 ? "bg-signal-red" : value >= 70 ? "bg-amber" : "bg-green";
  return (
...
function OwnedCard({ gym }: { gym: (typeof ownedGyms)[0] }) {
  return (
    <div className="bg-surface border border-border p-4 hover:border-gray-600 transition-colors group">
      <div className="flex items-start justify-between mb-3">
        <div className="w-9 h-9 bg-purple/20 flex items-center justify-center text-lg">
          🏟️
        </div>
        <span
          className={`text-[10px] px-2 py-0.5 border ${gym.license === "Open" ? "border-green/30 bg-green/10 text-green" : "border-purple/30 bg-purple/10 text-purple-400"}`}
        >
          {gym.license}
        </span>
      </div>
...
function MarketCard({
  ...
  gym: (typeof marketGyms)[0];
  purchasing: boolean;
  onPurchase: () => void;
}) {
  return (
    <div className="bg-surface border border-border p-4 hover:border-gray-600 transition-colors">
      <div className="flex items-start justify-between mb-3">
        <div className="w-9 h-9 bg-surface-2 flex items-center justify-center text-lg">
          {gym.category === "Coding"
            ? "💻"
            : gym.category === "Trading"
              ? "📈"
              : gym.category === "Physics"
                ? "⚛️"
        ...
        <span
          className={`text-[10px] px-2 py-0.5 border ${gym.license === "Open" ? "border-green/30 bg-green/10 text-green" : gym.license === "Enterprise" ? "border-amber/30 bg-amber/10 text-amber" : "border-purple/30 bg-purple/10 text-purple-400"}`}
        >
          {gym.license}
        </span>
      </div>
      ...
      <button
        onClick={onPurchase}
        className={`w-full text-[12px] py-1.5 font-medium transition-all ${
          purchasing

...
function OrchestratorNode({ data }: { data: ONodeData }) {
  ...
  return (
    <div
      className="relative border overflow-hidden min-w-36"
      style={{
    ...
    >
      ...
      <div className="px-3 py-2">
        ...
        {data.sub && (
          <div className="text-[10px] text-gray-500 mt-0.5">{data.sub}</div>
        )}
      </div>
      <Handle
        type="source"
...
const paletteItems: { type: ONodeType; label: string; desc: string }[] = [
  {
    type: "baseModel",
    label: "Base Model",
    desc: "Foundation LLM to fine-tune",
  },
...
const initialNodes: Node[] = [
  ...
  },
  {
    ...
    type: "orchestrator",
    position: { x: 340, y: 60 },
    data: { label: "PythonCoding", sub: "v3.1 · Open", nodeType: "gym" },
  },
  {
    id: "gy2",
    type: "orchestrator",
    position: { x: 340, y: 180 },
    data: { label: "MathEnv", sub: "v1.0 · Open", nodeType: "gym" },
  },
  {
    id: "sv1",
    type: "orchestrator",
    position: { x: 340, y: 300 },
    data: { label: "GitHub API", sub: "REST · OAuth", nodeType: "server" },
  },
  {
    id: "cp1",
    type: "orchestrator",
    position: { x: 600, y: 120 },
    data: { label: "4x A100 80G", sub: "320 GB VRAM", nodeType: "compute" },
  },
  {
    id: "dp1",
    type: "orchestrator",
    position: { x: 840, y: 120 },
    data: { label: "Deploy to Swarm", sub: "0G Network", nodeType: "deploy" },
  },
];
const initialEdges = [
  {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-[11px] text-muted">
              Active reinforcement learning sessions
            </p>
          </div>
          <button
            onClick={onNew}
        ...
        <div className="flex items-center gap-2 px-6 py-2.5 border-b border-border">
          {["All", "Running", "Paused"].map((f, i) => (
            <button
              key={f}
              className={`text-[11px] px-2.5 py-1 transition-colors ${i === 0 ? "bg-border text-white" : "bg-surface-2 text-muted hover:text-white"}`}
            >
              {f}
            </button>
          ))}
          ...
          <span className="text-[11px] text-muted">
            {arenas.filter((a) => a.status === "running").length} running ·{" "}
            {arenas.filter((a) => a.status === "paused").length} paused
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-space border-b border-border">
              <tr>
                {["Arena", "Model", "Gym", "Reward", "Loss", "Uptime"].map(
                  (h, i) => (
                    <th
                      key={h}
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <span
                        className={`w-1.5 h-1.5 shrink-0 ${a.status === "running" ? "bg-green animate-pulse" : "bg-amber"}`}
                      />
                      <div>
      ...
      <div className="w-64 border-l border-border bg-surface shrink-0 flex flex-col">
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <span
              className={`w-1.5 h-1.5 ${selected.status === "running" ? "bg-green animate-pulse" : "bg-amber"}`}
            />
            <span className="text-[11px] text-muted capitalize">
              {selected.status} · {selected.uptime}
            </span>
          </div>
        </div>
        <div className="p-4 space-y-4 flex-1 overflow-y-auto">
          <div className="grid grid-cols-2 gap-2">
            <Metric
              label="Reward"
              value={selected.reward.toFixed(1)}
              color="text-green"
            />
            <Metric
              label="Loss"
              value={selected.loss.toFixed(3)}
              color="text-amber"
            />
          </div>
          <div>
...
function Orchestrator({ onBack }: { onBack: () => void }) {
  ...
  const onConnect = useCallback(
    (c: Connection) =>
      setEdges((es) =>
        addEdge({ ...c, animated: true, style: { stroke: "#7c3aed" } }, es),
      ),
    [setEdges],
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-border bg-surface shrink-0">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 text-[12px] text-muted hover:text-white transition-colors"
        >
          ← Arenas
        </button>
        ...
        <span className="text-[11px] text-muted">
          Wire components to define your training run
        </span>
        <div className="flex-1" />
        <button
          onClick={deploy}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1" onDragOver={onDragOver}>
          <ReactFlow
            ...
            defaultEdgeOptions={{
              animated: true,
              style: { stroke: "#1e2030", strokeWidth: 2 },
            }}
          >
            ...
            <MiniMap
              style={{ background: "#0e1018", border: "1px solid #1e2030" }}
              nodeColor={(n) =>
                nodeColors[(n.data as ONodeData).nodeType] + "88"
              }
            />
          </ReactFlow>
        </div>

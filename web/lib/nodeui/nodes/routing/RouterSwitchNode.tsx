  ...
  return (
    <div
      ...
      style={{
        ...
        background: '#1a1a2e',
        borderRadius: 12,
        border: selected ? `1px solid ${ACCENT}66` : '1px solid #2a2a3e',
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 600, color: '#f0f0ff' }}>{d.label}</span>
        <button
          onClick={(e) => { e.stopPropagation(); removeNode(id) }}
          style={{ color: '#4a4a6a', background: 'none', border: 'none', cursor: 'pointer', padding: 2, lineHeight: 1 }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {d.conditions.map((c) => (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 10, color: '#f0f0ff', flex: 1 }}>{c.label}</span>
      ...
      <Handle
        ...
        id="exec-in"
        style={{ top: '50%', width: 10, height: 10, background: EXEC_COLOR, border: '2px solid #1a1a2e', borderRadius: '50%' }}
      />
      <span style={{ position: 'absolute', left: -8, top: '50%', transform: 'translate(-100%, -50%)', fontSize: 9, color: '#8888aa', whiteSpace: 'nowrap', pointerEvents: 'none' }}>
        Exec Flow
      </span>
        ...
        return (
          <div key={c.id}>
            <Handle
              ...
              id={`out-${c.id}`}
              style={{ top: `${pct}%`, width: 10, height: 10, background: ACCENT, border: '2px solid #1a1a2e', borderRadius: '50%' }}
            />
            <span style={{ position: 'absolute', right: -8, top: `${pct}%`, transform: 'translate(100%, -50%)', fontSize: 9, color: '#8888aa', whiteSpace: 'nowrap', pointerEvents: 'none' }}>
              {c.label}
            </span>
          </div>
        )
